...
from .config import JOB_ROOT, JOB_TTL_SECONDS, READ_FORMATS, WRITE_FORMATS
...
app = FastAPI(title="PandocAaS Worker", version="1.0.0")
...
def health() -> dict:
    ...
    return {
        "status": "ok",
        "pandoc": version,
        "tex_cache_warmed": tex_cache.exists(),
...
def convert(job: ConvertJob) -> ConvertResult:
    _cleanup_stale_jobs()
    if len(job.source.content.encode("utf-8")) > 25 * 1024 * 1024:
